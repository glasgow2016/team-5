...
@app.route("/posts/kelpie/<float:time><postId>")
def kelpie():
    finished = UpdateRoute(time, postId)
    print finished
    return render_template("kelpie.html")

#renders page delivered to user when they check in at visitor centre
@app.route("/posts/visitorCentre/<float:time><postId>")
def visitorCentre():
    finished = UpdateRoute(time, postId)
    print finished
...
#renders page delivered to user when they check in at visitor centre
@app.route("/posts/Lagoon/<float:time><postId>")
def Lagoon():
    finished = UpdateRoute(time, postId)
